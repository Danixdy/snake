...
window.title('Snake Game by group 5')                   #Ponemos titulo
...
cabeza.color('#FFFF00')                 #Color a la cabeza
...
def generar_color_segmento(indice, total):
    # Amarillo a Rojo
    if indice < total / 2:
        factor = indice / (total / 2)  # Proporción dentro de la mitad
        r = 255
        g = 223 - int(223 * factor)  # De amarillo a rojo (disminuye verde)
        b = 0
    # Rojo a Azul
    else:
        factor = (indice - total / 2) / (total / 2)  # Proporción dentro de la segunda mitad
        r = 255 - int(255 * factor)  # De rojo a azul (disminuye rojo)
        g = 0
        b = int(255 * factor)  # Incrementa azul
    return (r, g, b)


def actualizar_colores():
    
    total_seg = len(cuerpo)  # Cantidad actual de segmentos
    for i, segmento in enumerate(cuerpo):
        color = generar_color_segmento(i, total_seg)
...
def crearSegmento():
    ...
    total_seg = len(cuerpo) + 1
    color = generar_color_segmento(len(cuerpo), total_seg)
    segmento.color(color)
    segmento.penup()
